	...
	class WpssoOrg {
		...
		public static function required_notice( $deactivate = false ) {
			...
			if ( $deactivate === true ) {
				...
				wp_die( '<p>'.sprintf( __( '%1$s is an extension for the %2$s plugin &mdash; please install and activate the %3$s plugin before activating the %4$s extension.', 'wpsso-organization' ), $info['name'], $info['req']['name'], $info['req']['short'], $info['short'] ).'</p>' );
				...
				sprintf( __( 'The %1$s extension requires the %2$s plugin &mdash; please install and activate the %3$s plugin.',
					'wpsso-organization' ), $info['name'], $info['req']['name'], $info['req']['short'] ).'</p></div>';
		}
