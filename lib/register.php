...
if ( ! class_exists( 'WpssoOrgRegister' ) ) {
	...
	class WpssoOrgRegister {
		...
		public function __construct() {
			...
			//register_uninstall_hook( WPSSOORG_FILEPATH, array( __CLASS__, 'network_uninstall' ) );
		...
		private function activate_plugin() {
			$lca = 'wpssoorg';
			$version = WpssoOrgConfig::$cf['plugin'][$lca]['version'];	// only our config
			if ( class_exists( 'WpssoUtil' ) )
				WpssoUtil::save_all_times( $lca, $version );
			else WpssoOrg::wpsso_missing_notice( true );			// $deactivate = true
		}
